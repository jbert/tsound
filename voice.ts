...
export class Voice {
    ...
    Sound(fundamental: number, dur: number): sound.Sound {
        const harmonics = this.tones.map(
            (ampl: number, overtone: number): sound.Sound => {
                ...
                return sound.EvenLinearEnvelope(
                    snd,
                    [0.1, 1.0, 1.0, 0.7, 0.2, 0.1, 0.05, 0.05, 0.05, 0.05]
                );
            }
        );
...
export class TwangVoice {
    ...
    constructor(
        harmonicPower: number = 2,
        decayPower: number = 2,
        decreaseToBase: number = 0.05
    ) {
    ...
    Sound(fundamental: number, dur: number): sound.Sound {
        ...
        const harmonics = nums.map((idx: number): sound.Sound => {
            ...
            const decreaseTo =
                this.decreaseToBase / Math.pow(idx, this.decayPower);
