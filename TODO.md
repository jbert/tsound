...
- writing wireInput etc is fun but a chore. Move to react.
...
- DONE hook up multiple inputs

  - DONE feed to a gain node to mix
  - NO? add final gain node before destination to bring to [-1,+1] range

- configuration:

  - DONE freq
  - major/minor
  - live apply changes? (start/stop)

- create a 'guitarnode'

  - combine harmonics with mixer
...
DONE - move line + rect drawing over to 0,0 -> 1, 1 coords - bottom left origin
...
DONE - implement drawLine for SubScreen - consider abstracting the coord transformation?
...
DONE - play sound from JS - need to play wav....render it serverside? - could have PUT /foo.wav <wavdata> and then use the URL? - data:// url?
...
DONE - EnvelopeAmplitude - arb function - piecewise linear - expo decay
